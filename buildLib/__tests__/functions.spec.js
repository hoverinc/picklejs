(function (global, factory) {
    if (typeof define === "function" && define.amd) {
        define(['../functions', '../variables'], factory);
    } else if (typeof exports !== "undefined") {
        factory(require('../functions'), require('../variables'));
    } else {
        var mod = {
            exports: {}
        };
        factory(global.functions, global.variables);
        global.functionsSpec = mod.exports;
    }
})(this, function (_functions, _variables) {
    'use strict';
    ...
    describe('functions', () => {
        ...
        beforeEach(() => {
            _variables.ELEMENT_SELECTORS['Button'] = '.button';
            _variables.ELEMENT_SELECTORS['Input'] = '.input';
            _variables.ELEMENT_SELECTORS['Modal'] = {
                default: '.modal',
                Button: '.modal-button'
            };

            (0, _variables.setScreens)({
                'Home': '/home'
            });
            ...
            window = jest.fn().mockReturnValue({
                then: then.mockImplementation(func => {
                    func({ scrollTo });
        ...
        describe('getSelector', () => {
            ...
            it('an element and its parent', () => {
                expect(getSelector(['Modal', 'Button'])).toBe('[class*="modal"] [class*="modal-button"]');
            });
            ...
            it('Gets rid of \'s', () => {
                expect(getSelector(['Button\'s'])).toBe('[class*="button"]');
            });

            it('Makes an element singular if the singular option is passed in', () => {
                expect(getSelector(['Buttons'], { singular: true })).toBe('[class*="button"]');
            });
            ...
            it('selects the correct element if text is passed', () => {
                expect(getSelector('Button', { text: 'Hi There' })).toBe('[class*="button"]:contains("Hi There")');
            });
        });
        ...
        describe('type', () => {
            it('works with a random variable', () => {
                ...
                expect(typeFn).toBeCalledWith(expect.stringMatching(/user\d+/));
            });

            it('generates a random variable, saves it and then allows it to be used`', () => {
                type('user<rand:userId>', 'Input');

                expect(_variables.STATE.userId).toEqual(expect.stringMatching(/\d+/));

                type('user<var:userId>', 'Input');

                expect(typeFn).toHaveBeenCalledWith('user' + _variables.STATE.userId);
            });
        });
        ...
        test('takeSnapshot', () => {
            ...
            expect(matchImageSnapshot).toBeCalledWith('Test', {
                threshold: 1000,
                thresholdType: 'pixel'
            });
        });
        ...
        test('takeElSnapshot', () => {
            ...
            expect(matchImageSnapshot).toBeCalledWith('Input', {
                threshold: 1000,
                thresholdType: 'pixel'
            });
        });
        ...
        test('onPage', () => {
            ...
            expect(should).toBeCalledWith('contain', '/home');
        });
        ...
        test('nElements', () => {
            nElements(3, 'Input');
            expect(should).toHaveBeenCalledWith('have.length', 3);
        });
        ...
        test('elDoesNotExist', () => {
            ...
            expect(should).toHaveBeenCalledWith('have.length', 0);
        });

        test('elBackground', () => {
            elBackground('#ffffff', 'Input');

            expect(should).toHaveBeenLastCalledWith('have.css', 'background-color', 'rgb(255, 255, 255)');
        });

        test('elBorder', () => {
            elBorder('#ffffff', 'Input');

            expect(should).toHaveBeenLastCalledWith('have.css', 'border-color', 'rgb(255, 255, 255)');
        });
    });
});
